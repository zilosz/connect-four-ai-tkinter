...
class HomeScreen(tk.Frame):
    ...
    def __init__(self, app):
        ...
        self.title = tk.Label(
            self, text='Connect Four',
            bg=self.TITLE_BG, fg=self.TITLE_FG,
            font=(self.TITLE_FONT_NAME, title_font_size)
        )

        current_widget_y = 0
        self.title_window = self.canvas.create_window(
            title_x, current_widget_y, window=self.title,
            width=title_width, height=title_height, anchor='nw'
        )
        ...
        self.row_chooser = OptionChooser(
            self, 'Rows', current_widget_y, chooser_height,
        ...
        self.column_chooser = OptionChooser(
            self, 'Columns', current_widget_y, chooser_height,
            list(range(app.MIN_BOARD_SIZE, app.MAX_BOARD_SIZE + 1)),
            app.DEFAULT_COLUMNS
        )
        ...
        self.connect_amount_chooser = OptionChooser(
            self, 'Connect Amount', current_widget_y, chooser_height,
            list(range(app.MIN_CONNECT_AMOUNT, app.MAX_CONNECT_AMOUNT + 1)),
            app.DEFAULT_CONNECT_AMOUNT
        )

        current_widget_y += chooser_height + chooser_gap
        user_options = [Player.__name__, AI.__name__]
        self.user1_chooser = OptionChooser(
            self, 'User 1', current_widget_y, chooser_height,
            user_options, app.DEFAULT_USER_1.__name__
        )

        current_widget_y += chooser_height + chooser_gap
        self.user2_chooser = OptionChooser(
            self, 'User 2', current_widget_y, chooser_height,
            user_options, app.DEFAULT_USER_2.__name__
        )
        ...
        self.play_button = tk.Button(
            self, text='Play!', command=app.start_game, bg=self.PLAY_BUTTON_BG,
            fg=self.PLAY_BUTTON_FG, font=play_button_font
        )

        self.play_button.bind('<Enter>', self.on_play_button_hover)
        self.play_button.bind('<Leave>', self.on_play_button_leave)

        self.play_button_window = self.canvas.create_window(
            play_button_x, current_widget_y, window=self.play_button,
            width=play_button_width, height=play_button_height, anchor='nw'
        )
    ...
    def get_row_input(self):
        return int(self.row_chooser.get_option())

    def get_column_input(self):
        return int(self.column_chooser.get_option())

    def get_connect_amount_input(self):
        return int(self.connect_amount_chooser.get_option())

    def get_user1_input(self):
        return self.user1_chooser.get_option()

    def get_user2_input(self):
        return self.user2_chooser.get_option()
